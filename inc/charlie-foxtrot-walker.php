 ...
 public function start_lvl( &$output, $depth = 0, $args = array() ) {
  ...
  public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0) {
    ...
    if ( 0 === strcasecmp( $item->attr_title, 'mega-menu' ) && 1 === $depth ) {
        ...
        $output .= $indent . '<li role="columns" class="col-xs-6 col-md-4 text-center" id="menu_img_' . esc_attr($item->menu_order) . '" style="background: url(' . get_theme_mod('mega_menu_img_' . esc_attr($item->menu_order)) . '); background-position: center center; background-size: cover;">';
    //a element @$depth >=1
    $attributes = ! empty($item->attr_title) ? ' title="' . esc_attr($item->attr_title) . '"' : '';
    $attributes .= ! empty($item->target) ? ' target="' . esc_attr($item->target) . '"' : '';
    $attributes .= ! empty($item->xfn) ? ' rel="' . esc_attr($item->xfn) . '"' : '';
